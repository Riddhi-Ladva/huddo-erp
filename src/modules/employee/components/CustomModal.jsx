    ...
    >
      <div className={`bg-white w-full ${sizeClasses[size]} rounded-xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]`}>
        ...
        <div className="px-5 py-3.5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          <button 
            onClick={onClose}
            className="px-3.5 py-1.5 border border-slate-200 hover:border-slate-300 rounded-lg text-xs font-bold text-slate-700 bg-white transition-colors cursor-pointer"
          >
            {cancelText}
          </button>
          {onConfirm && (
            <button 
              onClick={onConfirm}
              className={`px-3.5 py-1.5 text-xs font-bold text-white rounded-lg transition-colors shadow-sm cursor-pointer ${
                isDestructive 
